...
const getStudentMarks = (req, res) => {
  ...
  const query = `
    SELECT s.id, s.name, s.email, s.phone, sm.ideation, sm.execution, sm.viva, sm.total_marks
    FROM students AS s
...
`;
  ...
  db.query(query, [studentId], (err, result) => {
    ...
    doc.text(`Idea Score: ${student.ideation}`, { align: "center" });
    doc.text(`Execution Score: ${student.execution}`, {
      align: "center",
    });
    doc.text(`Viva Score: ${student.viva}`, {
      align: "center",
    });
