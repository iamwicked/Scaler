...
const getAssignedStudents = (req, res) => {
  ...
  db.query(checkMentorQuery, (err, result) => {
    ...
    const getStudentsQuery = `
      SELECT students.id, students.name, students.email, students.phone, students.evaluated_by, student_marks.ideation, student_marks.execution, student_marks.viva, student_marks.total_marks
      FROM students
    ...
    db.query(checkStudentQuery, (err, result) => {
      ...
      db.query(checkMarkedQuery, (err, result) => {
        ...
        if (result.length > 0) {
          ...
          if (ideation !== undefined) {
            updateMarksQuery += ` ideation = ${ideation},`;
          }
          if (execution !== undefined) {
            updateMarksQuery += ` execution = ${execution},`;
          }
          if (viva !== undefined) {
            updateMarksQuery += ` viva = ${viva},`;
          }
          ...
          const addMarksQuery = `INSERT INTO student_marks (student_id, ideation, execution, viva) VALUES (${studentId}, ${
            ideation | null
          }, ${execution | null}, ${viva | null})`;
